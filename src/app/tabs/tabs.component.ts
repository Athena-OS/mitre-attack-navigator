...
export class TabsComponent implements AfterViewInit, OnDestroy {
    ...
    public cycleTheme() {
        const themeMapping = {
            ...
            'theme-override-dark': 'system',
            'theme-use-system': 'light',
        };
        const nextTheme = themeMapping[this.userTheme] || 'light';
        this.handleUserThemeChange(nextTheme);
    }

    /**
     * Get display name for current theme for tooltip
     */
    public getThemeDisplayName(): string {
        const displayNames = {
            'theme-override-light': 'Current theme: Light',
            'theme-override-dark': 'Current theme: Dark',
            'theme-use-system': 'Current theme: System',
        };
        return displayNames[this.userTheme] || 'Theme selector';
    }

    /** Returns canonical active theme key ("light"|"dark"|"system") from various userTheme values */
    public getActiveTheme(): 'light' | 'dark' | 'system' {
        const value = (this.userTheme || '').toLowerCase();
        if (value === 'theme-override-light' || value === 'light') return 'light';
        if (value === 'theme-override-dark' || value === 'dark') return 'dark';
        if (value === 'theme-use-system' || value === 'system') return 'system';
        return 'system';
    }

    /** Human-readable label for current theme */
    public getThemeLabel(): string {
        const label = this.getActiveTheme();
        return label.charAt(0).toUpperCase() + label.slice(1);
    }

    /** Helper: is theme option currently active? */
    public isThemeActive(theme: 'light' | 'dark' | 'system'): boolean {
        return this.getActiveTheme() === theme;
    }

    /** Open/close the theme options menu */
    public toggleThemeMenu(event?: MouseEvent): void {
        if (event) event.stopPropagation();
        this.themeMenuOpen = !this.themeMenuOpen;
    }

    /** Close theme menu on any document click */
    ...
    public setTheme(theme: 'light' | 'dark' | 'system'): void {
        this.handleUserThemeChange(theme);
        this.themeMenuOpen = false;
    }
